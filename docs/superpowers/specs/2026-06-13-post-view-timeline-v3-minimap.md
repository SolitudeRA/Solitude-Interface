...
- **高亮段**(`.pvp-minimap-window`,绝对定位):
    - `left` = `min(visibleIndices) / totalPosts`,`width` = `(max(visibleIndices) − min(visibleIndices) + 1) / totalPosts`(均为百分比)。
    ...
    - `transition: left / width <trans> <ease>`(随滚动平滑移动/伸缩)。

### 3.2 纯函数 `computeMinimapWindow`

```
computeMinimapWindow(visibleIndices: number[], totalPosts: number): { left: number; width: number }
```

- `totalPosts <= 0` 或 `visibleIndices` 为空 → `{ left: 0, width: 0 }`。
- 否则 `first = min`, `last = max`;`left = first / totalPosts * 100`;`width = (last − first + 1) / totalPosts * 100`(百分比,组件设为 inline `style.left/width = 'x%'`)。
...
| minimap 高亮段 | `--foreground` 68% · `opacity 0.5` · 无发光 · `left/width` = 可见范围占比 |
| 过渡           | `left/width` 380ms(沿用 `--trans`/曲线)                                   |
| 时间线本体     | 不变(K3 / J4 / sharp3.7 / 流光3.2s / 呼吸2.4s / morph 380ms)              |
| 已移除         | 进度数字、切换 pop                                                        |

## 5. 测试

- `computeMinimapWindow` 纯函数单测(满窗口、单篇可见、空、首尾、占比正确)。
