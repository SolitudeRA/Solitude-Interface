...
export default function DockTimelineContainer({
    ...
    // 处理点击主刻度跳转
    const handleScrollToPost = useCallback((index: number) => {
    ...
    const boundaryTickCounts = useMemo(() => {
        ...
        const indices = visibleIndices.length > 0 ? visibleIndices : posts.slice(0, 5).map((_, i) => i);
        ...
        if (indices.length > 0) {
            ...
            if (firstVisibleIndex !== undefined && firstVisibleIndex > 0) {
                ...
                const firstDateStr = dates[firstVisibleIndex] || posts[firstVisibleIndex]?.published_at;
                const prevDateStr = dates[prevIndex] || posts[prevIndex]?.published_at;
                if (firstDateStr && prevDateStr) {
                    const daysDiff = calculateDaysBetween(new Date(firstDateStr), new Date(prevDateStr));
        ...
        if (indices.length > 0) {
            ...
            if (lastVisibleIndex !== undefined && lastVisibleIndex < posts.length - 1) {
                ...
                const lastDateStr = dates[lastVisibleIndex] || posts[lastVisibleIndex]?.published_at;
                const nextDateStr = dates[nextIndex] || posts[nextIndex]?.published_at;
                if (lastDateStr && nextDateStr) {
                    const daysDiff = calculateDaysBetween(new Date(lastDateStr), new Date(nextDateStr));
    ...
    // 处理点击刻度行容器（包括gap区域）
    const handleTickRowClick = useCallback((e: React.MouseEvent<HTMLDivElement>) => {
        const container = containerRef.current;
        if (!container) return;
    ...
    const getTickScale = useCallback((tickIndex: number) => {
